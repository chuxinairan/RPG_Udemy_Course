...
public class Sword_Skill_Controller : MonoBehaviour
{
    ...
    private void BounceLogic()
    {
        if (isBouncing && bounceTargets.Count > 0)
        {
            ...
            {
                SwordEnemyDamage(bounceTargets[targetIndex].GetComponent<Enemy>());
                targetIndex++;
                bounceAmount--;
                ...
                if (hitTimer <= 0)
                {
                    ...
                    foreach (Collider2D collider in colliders)
                    {
                        if (collider.GetComponent<Enemy>() != null)
                            SwordEnemyDamage(collider.GetComponent<Enemy>());
                    }
                }
    ...
    #region Sword Controller
    private void BouncingFirstEnemyTouch(Collider2D other)
    {
    ...
    private void StuckInto(Collider2D other)
    {
        ...
        if(enemy != null)
        {
            ...
            if(player.skillMgr.swordSkill.timeStopUnlocked)
                enemy?.FreezeTimerFor(freezeDuration);

            if (player.skillMgr.swordSkill.vulnerableUnlocked)
    ...
    private void SwordEnemyDamage(Enemy enemy)
    {
        EnemyStats enemyStats = enemy.GetComponent<EnemyStats>();
        player.stats.DoDamage(enemyStats);
    }
